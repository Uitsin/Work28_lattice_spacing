  ...
  for (n = 0; n < nlocal; n++){
    ...
    for (m =0; m <nlocal+atom->nghost ; m++){
      ...
      if ( atype[atomi]== ROCK_ATOM_TYPE && atype[atomj] == CHANNEL_ATOM_TYPE ){ 
       	...
       	cond2 = ( (fabs(xj - xi)<1.e-3) && (fabs(yj - (yi -0.5*lattice_spacing) ) < 1.e-3) && (fabs(zj -zi) <1.e-3) );  
       	cond3 = ( (fabs(xj - xi)<1.e-3) && (fabs(yj - (yi +0.5*lattice_spacing) ) < 1.e-3) && (fabs(zj -zi) <1.e-3) ); 
       	cond4 = ( (fabs(xj - xi)<1.e-3) && (fabs(yj - yi) < 1.e-3) && (fabs(zj -(zi-0.5*lattice_spacing) ) <1.e-3) );  
       	cond5 = ( (fabs(xj - xi)<1.e-3) && (fabs(yj - yi) < 1.e-3) && (fabs(zj -(zi+0.5*lattice_spacing) ) <1.e-3) ); 
