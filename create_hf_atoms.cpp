...
CreateHfAtoms::CreateHfAtoms(LAMMPS *lmp) : Pointers(lmp) {}

/* ---------------------------------------------------------------------- */

void CreateHfAtoms::command(int narg, char **arg)
{
...
void CreateHfAtoms::add_lattice()
{
  ...
  for (k = klo; k <= khi; k = k + lattice_spacing)
    for (j = jlo; j <= jhi; j = j + lattice_spacing)
	  ...
	  cond1 = (y[0] < domain->boxlo[0] || y[0] >= domain->boxhi[0] ||
		   y[1] < domain->boxlo[1] || y[1] >= domain->boxhi[1] ||
		   y[2] < domain->boxlo[2] || y[2] >= domain->boxhi[2]); 
	  cond2 = (y2[0] < domain->boxlo[0] || y2[0] >= domain->boxhi[0] ||
		   y2[1] < domain->boxlo[1] || y2[1] >= domain->boxhi[1] ||
		   y2[2] < domain->boxlo[2] || y2[2] >= domain->boxhi[2]); 

	  cond3 = (y3[0] < domain->boxlo[0] || y3[0] >= domain->boxhi[0] ||
		   y3[1] < domain->boxlo[1] || y3[1] >= domain->boxhi[1] ||
		   y3[2] < domain->boxlo[2] || y3[2] >= domain->boxhi[2]); 

	  //	  cond2 = (y2[0] < sublo[0]-0.5 || y2[0] >= subhi[0] ||
	  //	   y2[1] < sublo[1]-0.5 || y2[1] >= subhi[1] ||
	  //	   y2[2] < sublo[2]-0.5 || y2[2] >= subhi[2]);
	  
	  
          if (!cond1) atom->avec->create_atom( CHANNEL_ATOM_TYPE ,y );
	  if (!cond2) atom->avec->create_atom( CHANNEL_ATOM_TYPE ,y2 );
	  if (!cond3) atom->avec->create_atom( CHANNEL_ATOM_TYPE ,y3 );

	  /*          if (y2[0] < sublo[0] || y2[0] >= subhi[0] ||
              y2[1] < sublo[1] || y2[1] >= subhi[1] ||
              y2[2] < sublo[2] || y2[2] >= subhi[2]) continue;
	  */
	  //	  atom->avec->create_atom(channel_type,y2);
