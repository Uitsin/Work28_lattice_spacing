...
DumpHfImage::DumpHfImage(LAMMPS *lmp, int narg, char **arg) :
...
void DumpHfImage::create_image()
{
  ...
  if (hf_flag){
    ...
    //  double xplot1[3];    double xplot2[3];
    int index;
  ...
  if (bondflag) {
    ...
    for (i = 0; i < nchoose; i++) {
      ...
      for (m = 0; m < n; m++) {
        ...
        if (bdiam == NUMERIC) {
          diameter = bdiamvalue;
	  if (bond_type[atom1][m]== SPRING_BOND_TYPE) diameter=diameter*bond_factor;
        } else if (bdiam == ATOM) {
          if (adiam == NUMERIC) {
            diameter = adiamvalue;
          } else if (adiam == TYPE) {
	...
	*/
	...
	if (bond_type[atom1][m]== WET_CHANNEL_BOND_TYPE){
	...
	delz = xplot_atom2[2] - xplot_atom1[2];
	  ...
	  */
          ...
          xmid[0] = xplot_atom1[1] + 0.5*dely;
          xmid[0] = xplot_atom1[2] + 0.5*delz;
	  ...
	  */
          ...
          xmid[0] = xplot_atom2[1] - 0.5*dely;
          xmid[0] = xplot_atom2[2] - 0.5*delz;
...
int DumpHfImage::pack_forward_comm(int n, int *list, double *buf, 
...
void DumpHfImage::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int DumpHfImage::modify_param(int narg, char **arg)
{
