...
BondRockRock::BondRockRock(LAMMPS *lmp) : Pointers(lmp) {}

void BondRockRock::command(int narg, char **arg)
{
  ...
  for (n = 0; n < nlocal; n++){
    ...
    */
    ...
    for (m =0; m <nlocal+atom->nghost; m++){
      ...
      cond0 = ( (fabs(xj - (xi - 1.0*lattice_spacing) )<1.e-6) && (fabs(yj - yi) < 1.e-6) &&  (fabs(zj - zi) < 1.e-6) );  // first neighbor 
      cond1 = ( (fabs(xj - (xi + 1.0*lattice_spacing) )<1.e-6) && (fabs(yj - yi) < 1.e-6) &&  (fabs(zj - zi) < 1.e-6) );  // second neighbor
      cond2 = ( (fabs(xj - xi)<1.e-6) && (fabs(yj - (yi - 1.0*lattice_spacing) ) < 1.e-6) &&  (fabs(zj - zi) < 1.e-6)  );
      cond3 = ( (fabs(xj - xi)<1.e-6) && (fabs(yj - (yi + 1.0*lattice_spacing) ) < 1.e-6) &&  (fabs(zj - zi) < 1.e-6)  );
      cond4 = ( (fabs(xj - xi)<1.e-6) && (fabs(yj - yi)  < 1.e-6) &&  (fabs(zj - (zi - 1.0*lattice_spacing)) < 1.e-6)  );
      cond5 = ( (fabs(xj - xi)<1.e-6) && (fabs(yj - yi)  < 1.e-6) &&  (fabs(zj - (zi + 1.0*lattice_spacing)) < 1.e-6)  );
...
/*------------------------------------------------------*/
int BondRockRock::pack_comm(int n, int *list, double *buf,
...
void BondRockRock::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
...
int BondRockRock::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void BondRockRock::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
double BondRockRock::memory_usage()
{
  int nmax = atom->nmax;
...
int * BondRockRock::sort(int *s, int n){
